  ...
  >
    <div class="flex flex-col gap-4">
      ...
      >
        ...
        <div class="flex flex-col gap-1">
          <label class="text-sm">{{ editForm.type === 'singbox' ? 'API Secret / Bearer Token' : t('password') }}</label>
          <input
            type="password"
            class="input input-sm w-full font-mono"
            v-model="editForm.password"
            :placeholder="editForm.type === 'singbox' ? 'Bearer Secret (leave blank if not set)' : ''"
          />
        </div>
      </div>
