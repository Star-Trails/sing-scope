...
const DEFAULT_BACKEND: Backend = {
  ...
  secondaryPath: '',
  password: '',
  uuid: 'default-singbox',
  label: 'sing-box (127.0.0.1:9090)',
}

export const backendList = useStorage<Backend[]>('setup/api-list', [DEFAULT_BACKEND])

// Ensure default backend exists
if (!backendList.value.length) {
  backendList.value = [DEFAULT_BACKEND]
}
...
export const activeUuid = useStorage<string>('setup/active-uuid', 'default-singbox')
export const activeBackend = computed(() =>
  backendList.value.find((backend) => backend.uuid === activeUuid.value) || backendList.value[0] || DEFAULT_BACKEND,
)
